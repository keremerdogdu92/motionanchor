...
fn inspect_workspace(workspace: &Path) -> Result<WorkspaceReadiness, String> {
    let missing_directories = REQUIRED_DIRECTORIES.iter()
        ...
        .collect::<Vec<_>>();
    let non_empty_output_directories = [workspace.join("artifacts/frames"), workspace.join("artifacts/rgba")]
        .into_iter()
        .filter_map(|path| match directory_is_non_empty(&path) { Ok(true) => Some(Ok(path.to_string_lossy().into_owned())), Ok(false) => None, Err(error) => Some(Err(error)) })
        .collect::<Result<Vec<_>, _>>()?;
    Ok(WorkspaceReadiness {
        workspace_path: workspace.to_string_lossy().into_owned(),
        ready: missing_directories.is_empty() && non_empty_output_directories.is_empty(),
        missing_directories,
        non_empty_output_directories,
        source_exists: workspace.join("media/source.mp4").is_file(),
        prompt_exists: workspace.join("prompts/sam2-prompts.json").is_file(),
    })
}
